...
func ValidatorHandler(w http.ResponseWriter, r *http.Request, grpcConn *grpc.ClientConn) {
	...
	go func() {
		...
		stakingRes, err := stakingClient.ValidatorDelegations(
			context.Background(),
			&stakingtypes.QueryValidatorDelegationsRequest{ValidatorAddr: address},
		)
		if err != nil {
			sublogger.Error().
				Str("address", address).
	...
	go func() {
		...
		slashingRes, err := slashingClient.SigningInfo(
			context.Background(),
			&slashingtypes.QuerySigningInfoRequest{ConsAddress: consAddress},
		)
		if err != nil {
			sublogger.Error().
				Str("address", address).
